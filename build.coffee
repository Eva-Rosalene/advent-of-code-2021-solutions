...
compileIndex = (days) ->
  ...
  indexHtml = mustache.render indexTemplate, days: preparedDays
...
minify = ->
  ...
  for file from files
    ...
    [error, data] =
      await tryToCatch minifier, "dist/#{file}",
        html:
...
main = ->
  ...
  days = entries.filter (entry) =>
    entry.startsWith('day-') and entry.endsWith '.coffee'
  days.sort (a, b) ->
