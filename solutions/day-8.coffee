...
decode = (patterns) ->
  ...
  for [segment, counter] from letters
    switch counter
      ...
      when 7 then sevens.add segment
        ...
        throw new Error(
          "impossible count of segments: segment #{segment} appeared #{counter} times"
        )
  ...
  for item from [...eights]
    if one.includes item
      mappings.set item, 'c'
      eights.delete item
  throw new Error 'eights must be of size 1 at this point' unless (
    eights.size is 1
  )
  ...
  for item from [...sevens]
    if four.includes item
      mappings.set item, 'd'
      sevens.delete item
  throw new Error 'sevens must be of size 1 at this point' unless (
    sevens.size is 1
  )
  mappings.set [...sevens][0], 'g'
  throw new Error 'mappings should be finished at this point' unless (
    mappings.size is 7
  )
  mappings
